...
plot.conso <- function(x,
  ...
  b <- max(x$vpress)
  a <- b/max(dtcurve$depths)
  dtcurve$depths <- -a * dtcurve$depths + b
  
  # set global plot ----
  delta_x <- diff(x$hour) * 0.1
  delta_y <- (depth(x) - min(dtcurve$depths)) * 1.7
  default_par <- list(
    col = 1, dive_col = 'dimgrey', type = "l", axes = FALSE,
    xlab = "Time (min)", ylab = "Pression (bar)", main = '',
    xlim = c(min(dtcurve$times) - delta_x, max(dtcurve$times) + delta_x),
    ylim = c(min(dtcurve$depths) - delta_y, max(dtcurve$depths) + delta_y)
  )
  ...
  if (!add) {
    ...
    # add depth lines ----
    if (line_print) {
      for (i in seq(from = 0, to = max(x$vpress), by = 10)) {
        abline(h = i, lty = 2, col = call_par$col, lwd = 0.1)
      }
      for (i in seq(from = 0, to = max(x$vpress), by = 50)) {
        abline(h = i, lty = 2, col = call_par$col, lwd = 0.3)
      }
    }
  }
  ...
  line_par <- c(list(x = dtcurve$times, y = x$vpress), call_par)
  line_par$xlim <- NULL ; line_par$ylim <- NULL
  line_par$xlab <- NULL ; line_par$ylab <- NULL
  line_par$main <- NULL ; line_par$axes <- NULL
  line_par$dive_col <- NULL
    ...
    for( i in x[c('time_mid', 'time_reserve', 'time_PA')]){
      if(is.numeric(i)){
        ...
        legend(posx[2], posy[2], x$lab[lab], text.col = call_par$col, 
               box.lty = 3, box.col = call_par$col, bg = par('bg'), 
               xjust = 1, cex = 0.8)
        # text(x = posx[2], y = posy[2], x$lab[lab], pos = 4, col = call_par$col )
      }
      lab = lab + 1
    }
  }
  ...
  # hour points ----
  points(x = c(x$dtcurve$times[1], tail(x$dtcurve$times, 1)), 
         y = rep(max(x$vpress), 2), pch = c(25, 24), 
         bg = rep(call_par$col, 2))
  if (hour_print){
    text(
      x = x$hour, y = max(x$vpress),
      minute_to_time(x$hour, sec = TRUE, sep = ':'),
      pos = 3, col = call_par$col
    )
  }
