...
check_val <- function(val){
  if(val < 0 | ! is.numeric(val)){
    stop(paste(deparse(substitute(val)), 
...
ndive <- function(dive1, dive2, inter = 16){
  ...
  if(inter <= 15){
    ...
    ndive <- list(dive1 = dive1, 
                  dive2 = dive(depth = depth, time = time, 
                               hour = dive1$hour[1]), 
    ...
    # compute maj
    maj <- majoration(depth = depth2, inter = inter, 
    ...
    if (tablecheck(depth2,time2+maj, force = TRUE) & 
      ...
      ndive = list(dive1 = dive1, dive2 = suc_dive, 
